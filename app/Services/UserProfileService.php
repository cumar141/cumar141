...
class UserProfileService
{
    ...
    {
        $user = User::with('user_detail', 'wallets.currency', 'user_detail.country:id')
    ...
    public function updateWebAccessState($state)
    {
        $user = User::where(['id' => auth()->user()->id])->first();
        $user->access_web = $state;
        $user->save();
        $response = [
            'message' => __('Web Access '. ($user->access_web ? "enabled" : "disabled") .'!'),
            'status'     => $user->access_web == $state
        ];
        return $response;
    }
    
    public function updateBiometricLoginState($state)
    {
        $user = User::where(['id' => auth()->user()->id])->first();
        $user->biometric_login = $state;
        $user->save();
        $response = [
            'message' => __('Biometric Login '. ($user->access_web ? "enabled" : "disabled") .'!'),
            'status' =>  $user->biometric_login == $state
        ];
        return $response;
    }
