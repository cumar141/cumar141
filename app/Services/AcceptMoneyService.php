...
class AcceptMoneyService
{
    ...
    public function cancel($trId, $user_id): bool
    {
        try {
            ...
            // $this->sendCancelNotificationToReceiver($requestPayment); SEND NOTIFICATION, CALL THE NEW SMS SERVICE LATER
            DB::commit();
            return true;
        } catch (Exception $e) {
            DB::rollback();
    ...
    {
        try {
            $transaction =  Transaction::where(['transaction_reference_id' => $trId, 'user_id' => $user_id])
    ...
    public function requestPayment($reference_id)
    {
        try {
            ...
            $requestPayment->status = 'Cancelled';
            $requestPayment->save();
            return $requestPayment;
        } catch (Exception $e) {
            throw new AcceptMoneyException(__('The :x does not exist.', ['x' => __('transaction')]));
