...
class RequestMoneyService
{
     ...
     * @throws RequestMoneyException
     */
    public function checkRequestSenderPhone($userId, $receiverPhone): object
    {
        $user     = User::where('id', '=', $userId)->first(['formattedPhone']);
        if (!$user) {
            throw new RequestMoneyException(__("User doesn't exists."));
        }
        $receiver = User::where('formattedPhone', '=', $receiverPhone)->first();
        ...
        if (!$receiver->identity_verified) {
            throw new RequestMoneyException(("Receiver account is not verified yet."));
        }
        if ($receiver) {
    ...
    public function store($emailOrPhone, $amount, $currency_id, $note, $userId, $processedBy)
    {
        ...
        $currency = Currency::find($currency_id, ['id', 'symbol']);
        if (!$currency) {
        ...
        try {
            ...
            $this->sendNotificationToAcceptor($transfer, true);
    ...
    public function sendNotificationToAcceptor($requestPayment, $new = false)
    {
        if($new) {
            $recipient = str_replace("+", "", $requestPayment->sender->formattedPhone);
            $data = [
                "{request_receiver_phone}" => is_null($requestPayment->receiver->formattedPhone) ? $requestPayment->receiver->email : $requestPayment->receiver->formattedPhone, 
                "{amount}" => moneyFormat(optional($requestPayment->currency)->symbol, formatNumber($requestPayment->amount, $requestPayment->currency_id)),
                "{request_sender_phone}" => is_null($requestPayment->sender->formattedPhone) ? $requestPayment->sender->email : $requestPayment->sender->formattedPhone,
                "{uuid}" => $requestPayment->uuid,
                "{created_at}" => dateFormat($requestPayment->created_at),
                "{soft_name}" => settings('name')
            ];
            $template = (new SmsTemplateService())->getSmsTemplate('notify-request-receiver');
