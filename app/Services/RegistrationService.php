...
class RegistrationService
{
    ...
    public function userRegistration($request)
    {
        try {
            ...
            $userPic = isset($request->image) ? (new UserProfileService())->uploadImage($user->id, $request->image) : null;
            RoleUser::insert(['user_id' => $user->id, 'role_id' => $user->role_id, 'user_type' => 'User']);
            $extras = isset($request->address_1) && isset($request->address_2) ? ["address_1" => $request->address_1, "address_2" => $request->address_2] : [];
            $this->user->createUserDetail($user->id, $extras);
