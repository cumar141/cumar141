...
class DepositMoneyService
{
    ...
    public function processPaymentConfirmation(
        ...
        try {
            ...
            } elseif (in_array($paymentMethodId, [Coinbase, Payeer, Coinpayments])) {
                $deposit = Deposit::success(
                    $currencyId,
                    $paymentMethodId,
                    $user_id,
            ...
            } else if (in_array($paymentMethodId, [PremierWallet, Somtel, TPlusWallet])) {
                $deposit = Deposit::success(
                    $currencyId,
                    $paymentMethodId,
                    $user_id,
                    ["totalAmount" => $totalAmount, "amount" => $amount, 'uuid' => $uuid, "reference_number" => $response["ReferenceNumber"], "external_reference" => $response["TransactionId"]], 
                    "Pending"
                );
                PaymentVerification::create(["platform" => $response["platform"], "transaction_id" => $response["TransactionId"], "uuid" => $uuid, "expires_at" => $response["expires_at"]]);
            } else {
                $deposit = Deposit::success(
                    $currencyId,
                    $paymentMethodId,
    ...
    public function processGatewayPayment($currencyId, $paymentMethodId, $amount, $bank_id)
    {
        ...
        if ($paymentMethodId == Bank) {
            ...
            $paymentData['banks'] = BankAccounts::where(['id' => $bank_id])->get(['id', 'bank_name', 'account_name', 'account_number']);
