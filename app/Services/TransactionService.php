...
class TransactionService
{
     ...
     */
    public function list($type="allTransactions", $userId, $offset = 0, $limit = 10, $order = "desc", $filters = []) : array
    {
        $data       = ['transactions' => [], 'totalRecords' => 0];
        $conditions = ['transactions.user_id' => $userId];
        
        if (!empty($filters) && isset($filters["transactions.end_user_id"])) {
            $conditions['transactions.end_user_id'] = $filters["transactions.end_user_id"];
        }
        
        if ('allTransactions' == $type) {
            $type = (new Transaction)::$transactionTypes;
        }else {
            $type = [TransactionType::where('name', $type)->first()->id];
        }
        
        $transaction = Transaction::with([
            'currency:id,type,code,symbol',
            'user:id,first_name,last_name,formattedPhone,picture',
            'end_user:id,first_name,last_name,formattedPhone,picture',
        ...
        if (!empty($filters) && isset($filters["date"])) {
        ...
        $transaction->orderBy('transactions.id', $order)
        ->select([
            ...
            'transactions.reference_number as reference_number',
