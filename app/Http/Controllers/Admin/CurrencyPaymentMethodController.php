...
class CurrencyPaymentMethodController extends Controller
{
    ...
    public function paymentMethodList($tab, $id)
    {
        ...
        $data['banks'] = BankAccounts::where(['currency_id' => $id])->get();
    ...
    public function getPaymentMethodsSpecificCurrencyDetails(Request $request)
    {
        ...
        if ($request->paymentMethod == Bank) {
            $banks = BankAccounts::where(['currency_id' => $request->currency_id])->get();
    ...
    public function addBank(Request $request)
    {
        ...
        if ($validation->passes()) {
            ...
            if ($bank->is_default == 'Yes') {
                BankAccounts::where(['is_default' => 'Yes', 'currency_id' => $bank->currency_id])->where('id', '!=', $bank->id)->update(['is_default' => 'No']);
            }
    ...
    {
        $bank = BankAccounts::where(['id' => $request->bank_id])->first();
    ...
    public function deleteBankLogo(Request $request)
    {
        if (isset($request->file_id)) {
            ...
            $bank = BankAccounts::where(['file_id' => $request->file_id])->first(['file_id']);

            if ($file && $bank) {
                BankAccounts::where(['file_id' => $request->file_id])->update(['file_id' => null]);
    ...
    public function updateBank(Request $request)
    {
        ...
        if ($validation->passes()) {
            ...
            $bank              = BankAccounts::find($request->bank_id);
            $bank->currency_id = $request->currency_id;
            $bank->country_id  = $request->country;
            $bank->admin_id    = auth('admin')->user()->id;
    ...
    public function deleteBank(Request $request)
    {
        ...
        $bank = BankAccounts::with('file:id,filename')->find($request->bank_id);
