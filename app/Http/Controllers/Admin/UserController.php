...
class UserController extends Controller
{
            ...
            if ($validator->fails()) {
                ...
                try {
                    DB::beginTransaction();
                        ...
                        RoleUser::insert(['user_id' => $user->id, 'role_id' => $user->role_id, 'user_type' => $role->customer_type]);
                   
                        $user->update(['type' => $role->customer_type]);
    ...
    public function edit($id)
    {
        ...
        $data['branch'] = Branch::select('id', 'name')->get();
    ...
    public function eachUserDepositSuccess(Request $request)
    {
        ...
        try
        {
            ...
            $deposit->balance           = $walletBalance + $amount;
            ...
            $transaction->balance                  = $walletBalance + $amount;
            $transaction->save();
    ...
    public function eachUserWithdrawSuccess(Request $request)
    {
        ...
        try
        {
            ...
            $withdrawal->balance           = $walletBalance - ($withdrawal->amount + $withdrawal->charge_percentage + $withdrawal->charge_fixed);
            ...
            $transaction->balance                  = $walletBalance - ($withdrawal->amount + $withdrawal->charge_percentage + $withdrawal->charge_fixed);
            $transaction->status                   = 'Success';
            $transaction->save();
