...
class MoneyTransferController extends Controller
{
    ...
    public function transferUserEmailPhoneReceiverStatusValidate(Request $request)
    {
        ...
        if ($phoneRegex)
        {
            ...
            if (in_array($request->receiver, [auth()->user()->formattedPhone, auth()->user()->phone1, auth()->user()->phone2, auth()->user()->phone3]))
            {
                return response()->json([
                    'status'  => true,
            ...
            $receiver = User::where('formattedPhone', $request->receiver)
                        ...
                        ->first(['status', 'identity_verified']);
            ...
            if (!empty($receiver))
            {
                ...
                if (!$receiver->identity_verified) {
                    return response()->json([
                        'status'  => true,
                        'message' => __("The recipient isn't verified"),
                    ]);
                }
            }
        }
        else
        {
            ...
            $receiver = User::where(['email' => trim($request->receiver)])->first(['status', 'identity_verified']);
            if (!empty($receiver))
            {
                if ($receiver->status == 'Suspended')
                {
    ...
    public function create(Request $request)
    {
        ...
        switch (preference('processed_by')) {
            case 'email':
                ...
                break;
            case 'phone':
                $placeHolder = __('Please enter valid :x', ['x' => __('phone (ex: +252000000000)')]);
                $helpText = __('We will never share your :x with anyone else.', ['x' => __('phone')]);
                break;
            case 'email_or_phone':
                $placeHolder = __('Please enter valid :x', ['x' => __('email (ex: user@gmail.com) or phone (ex: +252000000000)')]);
    ...
    public function store(Request $request)
    {
        ...
        if (!Hash::check($request->password, $user->password)) {
            return back()->withErrors(['password' => __('Password mismatch')])->withInput();
        }
        ...
        if ($request->sendMoneyProcessedBy == 'email') {
            ...
            if ($request->receiver[0] != "+" || !is_numeric($myStr[1])) {
                return back()->withErrors(__("Please enter valid phone (ex: +252000000000)"))->withInput();
            }
            ...
            if ($request->receiver[0] != "+" || !is_numeric($myStr[1])) {
                $rules['receiver'] = 'required|email';
                $messages          = [
                    'email' => __("Please enter valid email (ex: user@gmail.com) or phone (ex: +252000000000)"),
                ];
            }
        }
    ...
    public function sendMoneyConfirm(Request $request)
    {
        ...
        $transfer = (object) [
            "receiver" => $userInfo,
            "sender" => auth()->user(),
            "currSymbol" => $sessionValue['currSymbol'],
    ...
    public function notificationToSender($transfer)
    {
        ...
        $data = [
            "{receiver_id}" => getColumnValue($transfer->receiver),
            "{amount}" => moneyFormat($transfer->currSymbol, formatNumber($transfer->amount, $transfer->currency_id)),
