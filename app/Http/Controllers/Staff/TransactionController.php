...
use App\DataTables\Admin\StaffTransactionDataTable;
use Maatwebsite\Excel\Facades\Excel;
use App\Exports\staffTransactionExport;
class TransactionController extends Controller
{
    ...
    public function allTransactions(StaffTransactionDataTable $dataTable)
    {
        ...
        $data['status'] = isset(request()->transactionStatus) ? request()->transactionStatus : 'all';
        ...
        return $dataTable->render('staff.transactions.index', $data);
    }
    public function transactionPdf()
    {
        ...
        generatePDF('staff.transactions.transactions_report_pdf', 'transactions_report_', $data);
    }
    ...
    public function edit($id)
    {
        ...
        foreach ($modules as $module) {
            if (!empty(config($module->get('alias') . '.transaction_types'))) {
                if (in_array($transaction?->transaction_type->id, config($module->get('alias') . '.transaction_types'))) {
                    ...
                    return view($module->get('alias') . '::staff.transactions.edit', $data);
                }
            }
        }
        ...
        return view('staff.transactions.edit', $data);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->transaction_type_id == Deposit) {
            try {
                ...
                if ($request->status == 'Pending') {
                    if ($t->status == 'Pending') {
                        return redirect()->route('staff.transactions.all')->with('error', 'Transaction status is already ' . $request->status);
                ...
                return redirect()->route('staff.transactions.all')->with('success', 'Transaction status updated successfully.');
            } catch (\Exception $e) {
                \DB::rollBack();
                $this->helper->one_time_message('success', $e->getMessage());
                return redirect()->route('staff.transactions.all')->with('error', $e->getMessage());
            }
        }
        ...
        if ($request->transaction_type_id == Withdrawal) {
            if ($request->status == 'Success') {
                if ($t->status == 'Success') {
                    ...
                    return redirect()->route('staff.transactions.all')->with('success', 'Transaction status is succesfully updated.');
                } elseif ($t->status == 'Pending') {
                    $withdrawal = Withdrawal::updateStatus($request->transaction_reference_id, $request->status);
                    $this->transaction->updateTransactionStatus($request->id, $request->status);
                ...
                if ($t->status == 'Pending') {
                    ...
                    return redirect()->route('staff.transactions.all')->with('error', 'Transaction status updated successfully.' . $request->status);
        ...
        //Topup
        if ($request->transaction_type_id == 13) {
            if ($request->status == 'Success') {
                if ($t->status == 'Success') {
                    return redirect()->route('staff.transactions.all')->with('error', 'Transaction status is already ' . $request->status);
                } elseif ($t->status == 'Blocked') {
                    $topUp = TopUp::updateStatus($request->transaction_reference_id, $request->status);
                    $this->transaction->updateTransactionStatus($request->id, $request->status);
                    Wallet::deductAmountFromWallet($request->user_id, $request->currency_id, trim($request->total, '-'));

                    $data = [
                        'amount' => moneyFormat(optional($topUp->currency)->symbol, formatNumber(trim($request->total, '-'))),
                        'action' => 'added',
                        'fromTo' => 'to',
                        'user' => $topUp->user,
                        'type' => 'TopUp',
                    ...
                    (new TransactionUpdatedByAdminMailService())->send($topUp, $data);

                    if (!empty($topUp?->user?->formattedPhone)) {
                        (new TransactionUpdatedByAdminSmsService())->send($topUp, $data);
                    }
        ...
        if ($request->transaction_type_id == Transferred) {
            if ($request->status == 'Success') {
                if ($t->status == 'Success') {
                    ...
                    return redirect()->route('staff.transactions.all')->with('success', 'Transaction status is updated successfully');
                } elseif ($t->status == 'Blocked') {
                    $transfers = Transfer::with('transaction')->find($request->transaction_reference_id);
                    $transfers->status = $request->status;
                    $transfers->save();
        ...
        if ($request->transaction_type_id == Received) {
            if ($request->status == 'Success') {
                if ($t->status == 'Success') {
                    ...
                    return redirect()->route('staff.transactions.all')->with('error', 'Transaction status is aready ' . $request->status);
                } elseif ($t->status == 'Pending') {
                    $transfers = Transfer::with('transaction')->find($request->transaction_reference_id);
                    $transfers->status = $request->status;
                    $transfers->save();
                ...
                if ($t->status == 'Pending') {
                    return redirect()->rounted('staff.transactions.all')->with('error', 'Transaction status is already ' . $request->status);
                } elseif ($t->status == 'Success') {
                    $transfers = Transfer::with('transaction')->find($request->transaction_reference_id);
                    $transfers->status = $request->status;
                    $transfers->save();
        ...
        if ($request->transaction_type_id == Payment_Sent) {
            if ($request->status == 'Pending') {
                ...
                if ($t->status == 'Success') {
                    ...
                    if (!$merchant_payment) {
                        return redirect()->route('staff.transactions.all')->with('error', 'Transaction reference for merchant Payment not found');
                    }
                    $merchant_payment->status = $request->status;
                    $merchant_payment->save();
        ...
        if ($request->transaction_type_id == Payment_Received) {
            if ($request->status == 'Pending') {
                if ($t->status == 'Pending') {
                    ...
                    $merchant_payment = MerchantPayment::with('transaction')->find($request->transaction_reference_id);
                    if (!$merchant_payment) {
                        return redirect()->route('staff.transactions.all')->with('error', 'Transaction for merchant Payment  not found');
                    }
                    $merchant_payment->status = $request->status;
                    $merchant_payment->save();
        ...
        if ($request->transaction_type_id == Request_Sent) {
            if ($request->status == 'Success') {
                if ($t->status == 'Success') {
                    ...
                    return redirect()->route('staff.transactions.all')->with('success', 'Transaction status is already' . $request->status);
                }
            } elseif ($request->status == 'Refund') {
                if ($t->status == 'Refund') {
                ...
                if ($t->status == 'Pending') {
                    ...
                    if (!$request_payment) {
                        return redirect()->route('staff.transactions.all')->with('error', 'Transaction reference for request payment not found');
                    }
        ...
        if (module('Referral') && $request->transaction_type_id == Referral_Award) {
            ...
            if ($request->status == 'Pending') {
                return redirect()->route('staff.transactions.all')->with('error', 'Status not changed');
            }

            if ($request->status == 'Blocked' && $t->status == 'Success') {
                return redirect()->route('staff.transactions.all')->with('error', 'You can not change transactions status from Success to Blocked');
            }
    ...
    {
        return Excel::download(new staffTransactionExport(), 'transaction_list_' . time() . '.xlsx');
    }
