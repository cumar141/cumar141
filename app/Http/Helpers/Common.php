...
class Common
{
    ...
    {
        $selectOptions = ['id', 'first_name', 'last_name', 'email', 'carrierCode', 'phone', 'formattedPhone', 'picture'];
    ...
    */
    ...
    public function transactionFees($currencyId, $amount, $trasactionType, $paymentMethodId = null)
    {
        ...
        $fees->fees_percentage = round($amount * ($fees->charge_percentage / 100), 2);
        $fees->total_fees = round($fees->fees_percentage + $fees->charge_fixed, 2);
        $fees->total_amount = round($fees->total_fees + $amount, 2);
        return $fees;
    }
