...
function dateFormat($value, $userId = null) //$userId - needed for using user_id for mobile app (as mobile app does not know auth()->user()->id)
{
    ...
    if ($prefix == config('adminPrefix')) {
        ...
        if (!empty(auth()->user()->id) || !empty($userId)) {
            $timezone = $user?->user_detail?->timezone ?? "UTC";
        } else {
            $timezone = 'UTC';
        }
    }
...
function sendSMS($to, $message)
{
    ...
    if (!empty($smsConfig)) {
        $smsCredentials = json_decode($smsConfig, true);
        if (count($smsCredentials) > 0) {
            if ($smsConfig->provider == 'nexmo') {
                sendSMSwithNexmo($smsCredentials, $to, $message);
            }
            elseif ($smsConfig->provider == 'twilio') {
...
if (!function_exists('getStatusInputLabel')) {
    ...
    function getStatusInputLabel($type = null, $userType = null)
    {
        ...
        $transactionTypes = [
            ...
            Topup => ['user' => 'User', 'receiver' => 'Receiver'],
...
if (! function_exists('getPaymoneySettings')) {
    ...
    function getPaymoneySettings($type = null)
    {
        ...
        $array = [
            'company_info' => [
                'mobile' => [
                    'email' => "customercare@somxchange.com",
                    'phone' => "0611598869",
                    'whatsapp' => "+252611598869",
                    ]
                ],
            'transaction_types' => [
                'web' => [
                    'sent' => [Deposit, Transferred, Exchange_From, Exchange_To, Request_Sent, Withdrawal, Topup, Payment_Sent],
                    'received' => [Received, Request_Received, Payment_Received]
                ],
                'mobile' => [
                    'sent' => ['Deposit' => Deposit, 'Transferred' => Transferred, 'Exchange_From' => Exchange_From, 'Exchange_To' => Exchange_To, 'Request_Sent' => Request_Sent, 'Withdrawal' => Withdrawal, 'Payment_Sent' => Payment_Sent, 'Topup' => Topup],
            ...
            'payment_methods' => [
                'web' => [
                    ...
                    'withdrawal' => [Paypal, Bank, Crypto, Hormuud, Somtel, Amtel, PremierWallet],
                    'fiat' => [
                        'deposit' => [Mts, Stripe, Paypal, PayUmoney, Bank, Coinpayments, Payeer, Coinbase, Hormuud, Somtel, PremierWallet, TPlusWallet],
                        'withdrawal' => [Mts, Paypal, Bank, Hormuud, Somtel, Amtel, PremierWallet, Cash],
                    ],
                    'crypto' => [
                ...
                'mobile' => [
                    ...
                    'withdrawal' => ['Paypal' => Paypal, 'Bank' => Bank, 'Hormuud' => Hormuud, 'Somtel' => Somtel, 'Amtel' => Amtel, 'PremierWallet' => PremierWallet],
                    'fiat' => [
                        'deposit' => ['Stripe' => Stripe, 'Paypal' => Paypal, 'Bank' => Bank, 'Coinbase' => Coinbase, 'Coinpayments' => Coinpayments, 'Hormuud' => Hormuud, 'Somtel' => Somtel, 'PremierWallet' => PremierWallet, 'TPlusWallet' => TPlusWallet],
                        'withdrawal' => ['Hormuud' => Hormuud, 'Somtel' => Somtel, 'Amtel' => Amtel, 'PremierWallet' => PremierWallet, 'Paypal' => Paypal, 'Bank' => Bank],
                    ],
                    'crypto' => [
                        'deposit' => ['Coinpayments' => Coinpayments, 'Coinbase' => Coinbase, 'Hormuud' => Hormuud, 'Somtel' => Somtel],
...
if (!function_exists('getBankList')) {
    ...
    {
        $banks = BankAccounts::where(['currency_id' => $currencyId])->get(['id', 'bank_name', 'is_default', 'account_name', 'account_number']);
