...
class Transfer extends Model
{
    ...
    public function createTransferredTransaction($arr)
    {
        ...
        $transaction->balance           = ($arr['senderWallet']->balance - $arr['total']);
    ...
    public function createReceivedTransaction($arr)
    {
        if (isset($arr['userInfo'])) {
            $wallet = Wallet::firstOrCreate(
                ['user_id' => $arr['userInfo']->id, 'currency_id' => $arr['currency_id']],
                ['balance' => 0]
            );
        ...
        $transaction->balance           = ($walletBalance ?? 0) + $arr['amount'];
        $transaction->note              = $arr['note'];
        $transaction->status            = $arr['status'];
        $transaction->save();
