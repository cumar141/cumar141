...
class Deposit extends Model
{
    ...
    protected $fillable = ['user_id', 'currency_id', 'payment_method_id', 'bank_id', 'file_id', 'uuid', 'charge_percentage', 'charge_fixed', 'amount', 'balance', 'status'];
    ...
    public static function success($currencyId, $payment_method_id, $user_id, $sessionValue, $status="Success", $type="payment_gateway", $fileId=null, $bankId=null)
    {
        ...
        $reference_number = $sessionValue['reference_number'] ?? null;
        $external_reference = $sessionValue['external_reference'] ?? null;
        ...
        $deposit->balance           = $wallet->balance + $present_amount;
        $deposit->save();
        ...
        $transaction->balance                  = $wallet->balance + $present_amount;
        $transaction->save();
        if ($payment_method_id == Stripe || $payment_method_id == Paypal || $payment_method_id == Hormuud) {
