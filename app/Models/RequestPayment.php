...
class RequestPayment extends Model
{
    ...
    {
        $createWalletIfNotExist = Wallet::firstOrCreate(['user_id' => $user_id, 'currency_id' => $currency_id], ['balance' => 0]);
    }
    ...
    {
        $wallet = Wallet::firstOrCreate(['user_id' => $requestPayment->user_id, 'currency_id' => $arr['currency_id']], ['balance' => 0]);
        $balance = $wallet->balance + $arr["accept_amount"];
    ...
    {
        $wallet = Wallet::firstOrCreate(['user_id' => $requestPayment->receiver_id, 'currency_id' => $arr['currency_id']], ['balance' => 0]);
        ...
        $transaction->balance           = $wallet->balance - $t_total;
        $transaction->status            = 'Success';
        $transaction->save();
        return $transaction->id;
    }
    ...
    {
        $requestCreatorWallet           = Wallet::firstOrCreate(['user_id' => $requestPayment->user_id, 'currency_id' => $arr['currency_id']], ['balance' => 0]);
    ...
    {
        $requestAcceptorWallet          = Wallet::firstOrCreate(['user_id' => $arr['user_id'], 'currency_id' => $arr['currency_id']], ['balance' => 0]);
