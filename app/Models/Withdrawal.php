...
class Withdrawal extends Model
{
    ...
    public function createWithdrawal($arr)
    {
        ...
        $withdrawal->balance             = ($arr['wallet']->balance - $arr['totalAmount']);
    ...
    public function createWithdrawalDetail($arr)
    {
        ...
        $withdrawalDetail->type          = $arr['payment_method_id'];
        
        if ($arr['payment_method_id'] == Paypal) {
            $withdrawalDetail->email = $arr['payment_method_info']["email"] ?? "N/A";
        } elseif ($arr['payment_method_id'] == Bank) {
            $withdrawalDetail->account_name        = $arr['payment_method_info']["account_name"] ?? "N/A";
            $withdrawalDetail->account_number      = $arr['payment_method_info']["account_number"] ?? "N/A";
            $withdrawalDetail->bank_branch_name    = $arr['payment_method_info']["bank_branch_name"] ?? "N/A";
            $withdrawalDetail->bank_branch_city    = $arr['payment_method_info']["bank_branch_city"] ?? "N/A";
            $withdrawalDetail->bank_branch_address = $arr['payment_method_info']["bank_branch_address"] ?? "N/A";
            $withdrawalDetail->country             = $arr['payment_method_info']["country"] ?? "N/A";
            $withdrawalDetail->swift_code          = $arr['payment_method_info']["swift_code"] ?? "N/A";
            $withdrawalDetail->bank_name           = $arr['payment_method_info']["bank_name"] ?? "N/A";
        } elseif ($arr['payment_method_id'] == Crypto) {
            $withdrawalDetail->crypto_address = $arr['payment_method_info']["crypto_address"];
        } elseif (config('mobilemoney.is_active') && $arr['payment_method_id'] == (defined('MobileMoney') ? MobileMoney : '')) {
            $withdrawalDetail->mobilemoney_id = $arr['payment_method_info']["mobilemoney_id"] ?? "N/A";
            $withdrawalDetail->mobile_number = $arr['payment_method_info']["mobile_number"] ?? "N/A";
        }
    ...
    public function createWithdrawalTransaction($arr)
    {
        ...
        $transaction->end_user_id              = isset($arr['end_user_id']) ? $arr['end_user_id'] : NULL;
        ...
        $transaction->reference_number         = $arr['payment_method_info']["account_no"] ?? $arr['payment_method_info'] ?? "N/A";
        ...
        $transaction->note                     = isset($arr['note']) ? $arr['note'] : NULL;
