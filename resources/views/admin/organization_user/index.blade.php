...
@section('title', __('Organization User'))
    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="top-bar-title padding-bottom pull-left">{{ __('Organization User') }}</div>
                </div>

                <div>
                    @if(Common::has_permission(auth('admin')->user()->id, 'add_user'))
                        <a href="{{url(config('adminPrefix').'/organization/user/create/'. $organization_id)}}" class="btn btn-theme f-14"><span class="fa fa-plus"> &nbsp;</span>{{ __('Add Organization User') }}</a>
                    @endif 
                </div>
            </div>
        </div>
