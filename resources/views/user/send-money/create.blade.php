...
<div class="bg-white pxy-62 shadow" id="sendMoneyCreate">
    ...
    <form method="post" action="{{ route('user.send_money.store') }}" id="sendMoneyCreateForm">
        ...
        {{-- Password --}}
            <div class="col-md-12">
                <div class="label-top mt-20">
                    <label class="gilroy-medium text-gray-100 mb-2 f-15">{{ __('Password') }}</label>
                    <input type="Password" class="form-control input-form-control apply-bg l-s2" name="password" id="password" 
                    placeholder=""
                    onkeypress="return checkPassword(this, event);" required data-value-missing="{{ __('This field is required.') }}" value="{{ session('transInfo')['Password'] ?? old('Password') }}">
                    
                    @error('password')
                        <div class="custom-error">{{ $message }}</div>
                    @enderror
                    <label class="custom-error check-password-error"></span>
                </div>
            </div>
