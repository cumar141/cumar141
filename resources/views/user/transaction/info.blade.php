@foreach($transactions as $key=>$transaction)
    @php
        $transactionTypes = [Deposit, Withdrawal, Transferred, Received, Exchange_From, Exchange_To, Request_Sent, Request_Received, Payment_Sent, Payment_Received, Crypto_Sent, Crypto_Received, Topup];
    @endphp
        ...
        <div class="d-flex justify-content-between transac-child">
            <div class="d-flex w-50">
                ...
                <div class="deposit-circle d-flex justify-content-center align-items-center">
                    @php
                        if (in_array($transaction->transaction_type?->id, [Deposit, Withdrawal, Topup])) {
                ...
                <div class="ml-20 r-ml-8">
                    ...
                    <div class="d-flex flex-wrap">
                        @php
                            if (in_array($transaction->transaction_type?->id, [Deposit, Withdrawal,Topup, Payment_Sent, Payment_Received, Crypto_Sent])) {
                            ...
                            @if ($transaction->transaction_type_id == Withdrawal)
                                <div class="row gx-sm-5">
                                    <div class="col-6">
                                        ...
                                        <p class="mb-0 mt-5p text-dark gilroy-medium f-15 leading-22 r-text">{{ $transaction?->withdrawal->payment_method_info->account_no ?? $transaction?->withdrawal->payment_method_info ?? "-" }}</p>
                                    </div>
