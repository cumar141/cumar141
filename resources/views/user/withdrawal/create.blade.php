...
<div class="bg-white pxy-62 shadow" id="withdrawalCreate">
    ...
    <p class="mb-0 text-center f-14 gilroy-medium text-gray dark-p mt-20">
        {{ __('Accumulated wallet funds can simply be withdrawn at any time, to anywhere.') }}
    </p>
    ...
    <form method="post" action="{{ route('user.withdrawal.confirm') }}" id="withdrawalCreateForm">
        ...
        <div class="mt-20 param-ref">
            ...
            <div class="avoid-blink">
                {{-- used the currency model to get the code and id in order to populate the currency select  --}}
                <select class="select2" data-minimum-results-for-search="Infinity" name="currency_id" id="currency_id">
                    @foreach(\App\Models\Currency::active()->where('code', 'USD')->get(['id', 'code']) as $currency)
        ...
        <p class="mb-0 gilroy-medium f-12 mob-f-12 mt-select-to-p text-gray-100" id="walletHelp">{{ __('Fee') }}
            &nbsp;<span class="pFees">0</span>%+<span class="fFees">0</span>&nbsp;{{ __('Total Fee') }}: &nbsp;<span
                class="total_fees">0.00</span></p>

        <!-- Payment Methods -->
        <div class="mt-28 param-ref">
            <label class="gilroy-medium text-gray-100 mb-2 f-15">{{ __('Payment Method') }}</label>
            <div class="avoid-blink">
                <select class="select2" data-minimum-results-for-search="Infinity" name="withdrawal_method_id"
                    id="withdrawal_method_id">
                    @foreach ($withdrawalMethods as $method)
                    <option>{{ $method->payment_method->name }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="label-top mt-20">
            ...
            <input class="form-control input-form-control apply-bg l-s2" name="amount" id="amount"
                onkeypress="return isNumberOrDecimalPointKey(this, event);" placeholder="0.00" type="text"
                oninput="restrictNumberToPrefdecimalOnInput(this)" required
                data-value-missing="{{ __('This field is required') }}"
                value="{{ session('withdrawalData')['amount'] ?? old('amount') }}">
        </div>
